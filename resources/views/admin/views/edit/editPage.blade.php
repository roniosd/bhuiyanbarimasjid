<x-form.add-form title="Update Page" url="page" button="Update Page" :id="$page->id">
    ...
    <x-form.form-input label="Enter Slug" name="slug" id="unique_url" :value="$page->slug"
    ...
    <x-slot name="sitecontent">
        ...
        <x-form.form-select label="Enter your Template" name="template" id="template" :value="$page->template" :options="collect($templates)->mapWithKeys(fn ($template) => [$template => $template])->prepend('Choose Template', '')" />
